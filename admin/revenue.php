...
<head>
  ...
  font-size: 0.75rem; /* Even smaller font size for better visibility */
    ...
    @media print {
      ...
      .print-revenue-table, .print-revenue-table * {
        visibility: visible;
      }
      .print-revenue-table {
        position: absolute;
    ...
    <div class="content">
      ...
      <div class="mt-3">
        <div class="d-flex justify-content-between align-items-center mb-2">
          <h2 class="text-dark bg-light p-2 mb-0"><i class="bi bi-cash-stack"></i> Revenue Report</h2>
          <button class="btn btn-outline-dark btn-sm" onclick="window.print()">
            <i class="bi bi-printer"></i> Print Table
          </button>
        </div>
        <table class="table table-striped print-revenue-table table-bordered">
          <thead >
              <th>Month</th>
              <th>Revenue</th>
          </thead>
          <tbody>
              <?php
              include('../conn.php');
              $data = mysqli_query($conn, "SELECT YEAR(payment_date) AS year, MONTH(payment_date) AS month, SUM(amount) AS revenue FROM payments GROUP BY YEAR(payment_date), MONTH(payment_date) ORDER BY YEAR(payment_date), MONTH(payment_date)");
              while ($display = mysqli_fetch_assoc($data)) {
                $monthName = date("F", mktime(0, 0, 0, $display['month'], 10));
                $year = $display['year'];
